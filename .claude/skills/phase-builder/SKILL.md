...
- **Reporting the PR done without watching CI go green** — trusting a local sweep that can *false-green*. The classic case: the test harness provisions its schema from a hand-maintained DDL snapshot that drifted from the real migrations, so a *freshly-provisioned* CI database fails a test that a local database (which had accumulated the columns) passed. After pushing, `gh run watch <id> --exit-status` (or the platform equivalent) to green and fix-forward on red — the clean-checkout CI run is the ground truth, the local mirror only a proxy. Systemic fix for the drift class: have the test harness apply the real migrations, not a hand-mirrored snapshot.
