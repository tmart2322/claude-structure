...
- **Heavy commands run through the build-slot governor.** Prefix every toolchain burst — dependency install, tests, typecheck, lint, builds, the gate sweep — with `.claude/scripts/with-build-slot.sh` (e.g. `.claude/scripts/with-build-slot.sh <the project's test command>`). Several agent fleets can share one machine, and ungoverned bursts have memory-exhausted and kernel-panicked a real operator box; the wrapper queues until a machine-wide slot frees. A `[build-slot] waiting` log is normal — never bypass the wrapper because a wait looks slow. Plain `git`, greps, and file edits don't need it.
