...
- **Heavy runs go through the build-slot governor.** Prefix the e2e suite, local-stack boots, builds, and installs with `.claude/scripts/with-build-slot.sh` — several agent fleets can share one machine and ungoverned bursts have kernel-panicked a real operator box. A `[build-slot] waiting` log is normal queuing, not a failure. One-off screenshots and API probes don't need it.
